...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ========================================
  // NEW: Gemini-powered contract analysis endpoint
  // This endpoint connects to analyze_pdf.py for AI-powered analysis
  // ========================================
  app.post("/api/contracts/gemini-analyze", async (req, res) => {
    try {
      if (!req.user) {
        return res.status(401).json({ message: "Not authenticated" });
      }

      const { contractId, userPrompt, analysisType } = req.body;
      ...
      // Get contract details from database to find file path
      ...
      const args = [pythonScript, contractPath];
      
      // Add user prompt if provided, otherwise it will do general analysis
      if (userPrompt && userPrompt.trim()) {
        args.push(userPrompt);
      } else if (analysisType) {
        // Convert analysis type to a specific prompt
        const typePrompts: Record<string, string> = {
          'buyer': 'Analyze this contract from the buyer\'s perspective. Focus on risks and protective clauses for the service recipient.',
          'seller': 'Analyze this contract from the seller\'s perspective. Focus on obligations and liabilities for the service provider.',
          'general': 'Provide a comprehensive analysis of this contract, highlighting key terms, risks, and recommendations.'
        };
        const prompt = typePrompts[analysisType] || typePrompts.general;
        args.push(prompt);
      }

      console.log('Executing Gemini analysis:', {
        contractId,
        filePath: contract.filePath,
        userPrompt: userPrompt || 'General analysis',
        command: `python ${args.join(' ')}`
      });

      // Execute the Python script
      const result = await new Promise<string>((resolve, reject) => {
        exec(`python ${args.map(arg => `"${arg}"`).join(' ')}`, 
          { 
            maxBuffer: 1024 * 1024 * 10, // 10MB buffer for large responses
            env: { ...process.env } // Pass environment variables including GEMINI_KEY
          },
          (error, stdout, stderr) => {
            if (error) {
      ...
      try {
        ...
        // Save the chat message to database for history
        if (userPrompt && userPrompt.trim()) {
          // Save user's question
          await storage.createContractChat({
            contractId,
            userId: req.user.id,
            message: userPrompt,
            isSystem: false,
          ...
          await storage.createContractChat({
            ...
            isSystem: true,
            inputTokens: response.stats?.prompt_tokens || 0,
            outputTokens: response.stats?.output_tokens || 0
          });
        }
