          ...
          <nav className="px-3 pb-3">
            ...
            <div className="mt-6 pt-6 border-t border-gray-700">
              <h3 className={cn(
                ...
                {!isSidebarCollapsed && t('المساعدة', 'Help')}
              </h3>
              <div className="space-y-1">
                <button
                ...
                >
                  <Star className="w-4 h-4 text-gray-400 group-hover:text-white flex-shrink-0" />
                  {!isSidebarCollapsed && <span className="text-sm">{t('ما الجديد', "What's New")}</span>}
                </button>
                
                <button
                  onClick={() => toast({ title: t('قريباً', 'Coming Soon'), description: t('تقديم الملاحظات قريباً', 'Feedback coming soon') })}
                  className={cn(
                    "w-full flex items-center gap-3 py-2 rounded hover:bg-gray-700 transition-colors group",
                    isSidebarCollapsed ? "justify-center px-2" : "px-3"
                  )}
                  title={isSidebarCollapsed ? t('تقديم الملاحظات', 'Feedback') : undefined}
                >
                  <MessageSquare className="w-4 h-4 text-gray-400 group-hover:text-white flex-shrink-0" />
                  {!isSidebarCollapsed && <span className="text-sm">{t('تقديم الملاحظات', 'Feedback')}</span>}
                </button>
              </div>
            </div>
          </nav>
